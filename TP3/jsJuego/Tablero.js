class Board{
    ...
    createSpots(xstart, ystart) {
        ...
        for (let col = 0; col < this.mode.columns; col++) {
            ...
            for (let row = 0; row < this.mode.rows; row++) {
                let spot = new Casillero(posx, posy, this.context, this.spotSize, this.spotSize);
                column.push(spot);
    ...
    createArrows(start, width) {
        let arrowsRow = [];
        for (let col = 0; col < this.mode.columns; col++) {
            let arrow = new Flecha(start, this.ygap, this.context, start, this.spotSize/2, width, "#C0C6FA");
    ...
    createPieces(player, startx) {
        ...
        for (let i = 0; i < this.mode.pieces; i++) {
            let piece = new Ficha(x, y, this.context, radious, player);
            y -= this.margin;
            pieces.push(piece);
        }
        return pieces;
