...
//variables globales
let current_mode = CuatroEnLinea;  //Por defecto
let player1;
let player2;
let board;
let current_player;
...
function selectPiece(selectedBtn, btnsContainer, player) {
    ...
    if(player1 == player2) {
        showErrorMsg("Selecciona una ficha distinta.");
        for (const btn of btnsContainer.children) {
            btn.classList.remove("piece-settings-btn-active");
        }
...
function setFormBtnsEvents() {
    ...
    let radioInputs = document.querySelectorAll("input");
        ...
        input.addEventListener("click", () => {
            ...
            selectMode(radioInputs, name_current_mode); // hace que distinga un input de otro y resalte color
        });
    });
...
function setBoard(name_current_mode) {
    if(name_current_mode === "")
        current_mode = CuatroEnLinea;
    else if(name_current_mode === "CincoEnLinea")
        current_mode = CincoEnLinea;
    else if(name_current_mode === "SeisEnLinea")
        current_mode = SeisEnLinea;
    else if(name_current_mode === "SieteEnLinea")
        current_mode = SieteEnLinea;
}


// inicializa el juego
document.querySelector("#btn-juego").addEventListener("click", renderizarJuego);
...
conteinerCanvas.classList.add("container-inactivo");
...
function checkSettings() {
    if(player1 == null && player2 == null) {
        showErrorMsg("Selecciona una ficha");
        return false;
    } 
    if(player1 == null || player2 == null) {
        showErrorMsg("Selecciona otra ficha");
        return false;
    } 
    return true;   
}
